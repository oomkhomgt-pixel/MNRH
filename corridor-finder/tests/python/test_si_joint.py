...
def _pelvis(right_gap=3.0, left_gap=6.0, wide_behind=True):
    ...
    the interosseous ligament's space, which is not the joint."""
    ...
    right = (xx >= 70 + right_gap) & (xx <= 100) & (yy >= 20) & (yy <= 60) & (zz >= 10) & (zz <= 50)
    if wide_behind:
        right &= ~((yy < 30) & (xx < 82))  # the posterior third, 12 mm across
...
def test_each_joint_is_measured_at_the_s1_s2_level():
    ...
    assert widths["right"].n_samples > 100 and widths["left"].n_samples > 100
    ...
    assert widths["right"].sentence() == "right SI joint 3.0 mm"
