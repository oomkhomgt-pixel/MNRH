...
| 7.3a | The **antegrade** route starts **on the brim behind the iliopectineal eminence**, and its tip stops in the ischium or anywhere up to the ischial spine, as the surgeon chooses: the target region covers that whole run and the length is offered as options. How far behind the eminence is set at 60 mm, measured on four hemipelves (nearer than about 50 mm every axis crosses the acetabulum and reads 1-3 mm wide; from 55 mm back a 7.3 mm screw has 2.5-4.3 mm of clearance). **Awaiting the surgeon's confirmation of that distance.** |
